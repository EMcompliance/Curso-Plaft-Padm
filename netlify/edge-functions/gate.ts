...
function brandBlockHtml(brand?: ClientRecord | null): string {
  if (brand?.logo) {
    const nameLine = brand.displayName ? `<div class="name">${esc(brand.displayName)}</div>` : "";
    return `<div class="brand-logo"><img class="logo" src="${esc(brand.logo)}" alt="${esc(brand.displayName || "Logo")}">${nameLine}</div>`;
  }
...
function brandColor(brand?: ClientRecord | null): string {
...
<style>
  ...
  .brand-row .badge{width:32px;height:32px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-family:ui-monospace,'SF Mono',Menlo,Consolas,'Liberation Mono',monospace;font-weight:700;font-size:11px;color:#fff;flex-shrink:0;}
  .brand-row .name{font-size:13px;font-weight:600;color:#0B1F33;}
  .eyebrow{font-family:ui-monospace,'SF Mono',Menlo,Consolas,'Liberation Mono',monospace;font-size:10.5px;font-weight:700;letter-spacing:1.3px;text-transform:uppercase;color:${accent};margin-bottom:8px;}
  ...
  button{width:100%;padding:12px;border-radius:8px;border:none;background:${accent};color:#fff;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;}
  .pw-wrap{position:relative;margin-bottom:14px;}
  .pw-wrap input{margin-bottom:0;padding-right:46px;}
  .pw-toggle{position:absolute;top:0;right:0;height:100%;width:44px;padding:0;background:none;border:none;color:#5C6B7A;cursor:pointer;display:flex;align-items:center;justify-content:center;}
  .pw-toggle svg{width:20px;height:20px;}
  .pw-toggle:hover,.pw-toggle.active{color:${accent};}
  ...
  .footnote-row{display:flex;align-items:center;gap:6px;font-size:10.5px;color:#5C6B7A;font-family:ui-monospace,'SF Mono',Menlo,Consolas,'Liberation Mono',monospace;letter-spacing:.3px;}
  ...
  .thumb{width:34px;height:34px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-family:ui-monospace,'SF Mono',Menlo,Consolas,'Liberation Mono',monospace;font-weight:700;font-size:11px;color:#fff;flex-shrink:0;object-fit:cover;}
  .client-row-main .info{flex:1;min-width:120px;}
  .client-row-main .info .dname{font-size:13.5px;font-weight:600;}
  .client-row-main .info .slug{font-family:ui-monospace,'SF Mono',Menlo,Consolas,'Liberation Mono',monospace;font-size:11px;color:#8a97a3;}
...
<form method="POST" action="${esc(opts.action)}">
  ...
  <div class="pw-wrap">
    <input type="password" name="password" id="loginPw" placeholder="Contraseña" autofocus required>
    <button type="button" class="pw-toggle" id="loginPwBtn" onclick="togglePw()" aria-label="Mostrar contraseña">
      <svg id="pwIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
    </button>
  </div>
  <button type="submit">Entrar</button>
</form>
<div class="footnote">
  <div class="footnote-row"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg><span>Conexión cifrada &middot; HTTPS</span></div>
  <div class="footnote-row"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"></rect><path d="m22 6-10 7L2 6"></path></svg><a href="mailto:info@emcomplianceuy.com">info@emcomplianceuy.com</a></div>
</div>
<script>
  function togglePw() {
    var el = document.getElementById('loginPw');
    var btn = document.getElementById('loginPwBtn');
    var icon = document.getElementById('pwIcon');
    if (el.type === 'password') {
      el.type = 'text';
      btn.classList.add('active');
      btn.setAttribute('aria-label', 'Ocultar contraseña');
      icon.innerHTML = '<path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-7 0-11-8-11-8a18.5 18.5 0 0 1 5.06-5.94"></path><path d="M9.9 4.24A10.94 10.94 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"></path><path d="M14.12 14.12a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line>';
    } else {
      el.type = 'password';
...
export default async (req: Request, context: Context) => {
  ...
  if (path === AUTH_PATH && req.method === "POST") {
  ...
  if (path === ADMIN_CLIENTS_PATH || path === ADMIN_CLIENTS_API) {
    ...
    if (path === ADMIN_CLIENTS_API && req.method === "POST") {
      const form = await req.formData();
